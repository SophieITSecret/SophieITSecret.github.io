# BARソフィー 画面構成ツリー（2026-06-30 更新）

> 2026-06-30 更新点：ツンデレ動的メニュー、音楽サブメニュー（DJソフィー／音楽リクエスト）、戻るボタン「1つ上」ルール、PWA更新トーストを反映。
...
**会員・ログイン済み**
        ...
        　　※ カウンター内で何か操作すると、この案内タイマーは即解除（鳴らない）。何もしない時だけ鳴る。

### C のメニュー表示モード（localStorage: `sophie_menu_mode`）
- **ツンデレモード（既定）**：最初は「ソフィーと話す」ボタンのみ表示 → タップで動的展開（下記）
- **クラシックモード／音楽再生中**：4ボタン（ソフィーと話す／音楽／お酒／NEWS）を直接表示
...
│
├─ S0　音楽メニュー（DJソフィー／音楽リクエスト）　🆓　← クラシック「音楽」ボタン／ツンデレ「歌」から
│       　　<span style="color:#e8607a">Ｔ　入室時</span>
│       　　<span style="color:#e8607a">　　上位メニュー（C）から入った時</span>　<span style="color:#27ae60">sophie_music_intro.mp3</span>
│       　　<span style="color:#e8607a">　　下の画面（DJ／曲リスト）から戻ってきた時</span>　<span style="color:#27ae60">sophie_music_back.mp3</span>
│       ├─ D1　DJソフィーの歌とお酒の物語　🆓
│       │       　　<span style="color:#e8607a">Ｔ　入室時</span>　<span style="color:#27ae60">sophie_dj_intro.mp3</span>
│       │       └─ D2　エピソード一覧（🔁 連続再生トグル）
│       │               └─ D3　エピソード再生（ソフィー解説 ＋ YouTube歌・スライド）
│       └─ S1　音楽リクエスト：歌手リスト　🆓
│               　　<span style="color:#e8607a">Ｔ　1秒後</span>
...
## 2026-06-30 追加・変更まとめ
- **ツンデレ動的メニュー**：`sophie_menu_mode`（tsundere=既定／classic）。⚙管理者トグルで切替。音楽再生中はクラシック4ボタン。
  - 追加ナレーション：`sophie_counter_offer` / `sophie_counter_wrong1` / `sophie_counter_wrong2` / `sophie_counter_oshioki` / `sophie_counter_menu`
- **S0 音楽メニュー（新設）**：C と S1 の間の中間画面。DJソフィー（D1〜D3）と音楽リクエスト（S1）を選択。
  - 追加ナレーション：`sophie_music_intro`（上位から）／`sophie_music_back`（下から戻り）／`sophie_dj_intro`（DJ入室）
- **戻るボタンは常に1つ上**：DJエピソード一覧（D2）・音楽の歌手/曲リスト（S1/S2）の戻るは **S0** へ戻る（root へ飛ばない）。
- **ナレーションの停止徹底**：ユーザー操作が入ると再生中のセリフは必ず停止（`_stopNarration`）。アイドル案内 `member_caution_01` も操作で即解除。
- **PWA更新**：`sw.js` の `SW_VERSION` と `index_m.html` の `app_m.js?v=N` をデプロイ毎に揃えて上げる。新版があると更新トーストで通知（前面復帰時 `visibilitychange` でも検知）。配信は Firebase Hosting（`firebase deploy --only hosting`）。
