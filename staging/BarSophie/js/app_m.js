...
import { AXIS4_MAP, AXIS4_DEFAULT } from './axis4_map.js';
...
function initScrState() {
    scrState = {
        ...
        cospa: "", isStandard: "", isSophieRecom: "", // 追加条件
        pMin: "", pMax: "", aMin: "", aMax: "",
...
// ★第4軸・動的ダブルスライダー生成関数 [cite: 1138, 1141, 1144, 1150, 1151]
...
function openScreeningUI() {
    ...
    // ★第4軸の動的表示 [cite: 1132, 1133, 1135]
    const axis4 = scrState.sub ? (AXIS4_MAP[scrState.sub] ?? AXIS4_DEFAULT) : AXIS4_DEFAULT;
...
// ★スクリーニング結果の描画（スクロール維持対応） [cite: 1191, 1197, 1205]
...
function executeScreening() {
    ...
    const results = nav.liquorData.filter(d => {
        ...
        if(scrState.sub) { // 中分類指定時のみ第4軸をチェック [cite: 1177]
    ...
    renderSearchResults(results); // 切り出した関数を呼ぶ [cite: 1220]
}
...
function showLiquorCard(globalIndex, currentListArray = null) {
    ...
    // 第3軸・第4軸はClaudeのみ表示 [cite: 1164]
    h += makeInlineGraph("常道", "独特", "", "", d["Claude_個性"], true);
    const cardAxis4 = AXIS4_MAP[d["中分類"]] ?? AXIS4_DEFAULT;
...
function handleBack() {
    if (nav.state === "lq_card") {
        if(nav.curP && nav.curP.length > 0 && !nav.curP[0]["中分類"]) {
            renderSearchResults(nav.curP, nav.curI); // 検索結果のスクロール位置を復元 [cite: 1234]
        } else {
