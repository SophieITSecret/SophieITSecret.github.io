    ...
    if(sophieWarp) {
        sophieWarp.onclick = () => { 
            if(nav.state !== "none") { showRootMenu(); } 
                ...
                try { talkAudio.play(); } catch(e) { finalizeExit(); }
            } 
        };
    }
...
function prep(t, isM, id = null, originalTxt = null) {
    ...
    if(isM) { setTimeout(() => { if(lastTxt === t) tel.style.display = 'none'; }, 5000); } 
        ...
        try { talkAudio.play(); } catch(e) { try { media.speak(speakTxt); } catch(err){} }
    }
...
function attachMultiSlider(id) {
    ...
    if(minEl && maxEl) { minEl.addEventListener('input', update); maxEl.addEventListener('input', update); update(); }
}
...
function openScreeningUI() {
    ...
    h += `${makeDoubleSliderWithTitle('scr-s4', axis4.label, axis4.left, axis4.right, scrState.s4Min, scrState.s4Max, !scrState.sub)}</div>`;
    ...
    document.getElementById('scr-mj').onchange = (e) => { saveScrFormState(); scrState.major = e.target.value; scrState.sub = ""; scrState.area = ""; openScreeningUI(); };
    document.getElementById('scr-sb').onchange = (e) => { saveScrFormState(); scrState.sub = e.target.value; scrState.area = ""; openScreeningUI(); };
    document.getElementById('scr-ar').onchange = (e) => { scrState.area = e.target.value; };
}

// ★スクリーニング結果表示
function renderSearchResults(results, scrollToGlobalIdx = null) {
    nav.updateNav('lq_res', null, results); // curGをnullにすることで検索モードであることを記憶
    ...
    if(scrollToGlobalIdx !== null) {
        setTimeout(() => {
            const target = document.querySelector(`[data-lqidx='${scrollToGlobalIdx}']`);
...
function executeScreening() {
    ...
    const results = nav.liquorData.filter(d => {
        ...
        if(scrState.cospa) {
            const cCount = (d["Gemini_コスパ"]||"").split('☆').length - 1;
        ...
        if(scrState.area) {
            const itemArea = (d["国"] && d["産地"]) ? `${d["国"]} / ${d["産地"]}` : d["国"];
        ...
        let abv = parseFloat(d["度数"] || "0");
        if (abv > 0 && abv <= 1.0 && !(d["度数"]||"").includes('%')) abv *= 100;
...
function openLiquorList(subListArrayOrSubName) {
    let items, title;
    if(Array.isArray(subListArrayOrSubName)) { items = subListArrayOrSubName; title = items[0]["中分類"]; }
    else { items = nav.liquorData.filter(d => d["中分類"] === subListArrayOrSubName); title = subListArrayOrSubName; }
    nav.updateNav("lq_list", nav.curG, items); 
    let h = `<div class="label">${title} 銘柄一覧</div>`;
    items.forEach(d => { h += `<div class="item" data-lqidx="${nav.liquorData.indexOf(d)}">🥃 ${(d["銘柄名"]||"").replace(/"/g, '')}</div>`; });
...
function showLiquorCard(globalIndex, currentListArray = null) {
    ...
    let tagsHtml = "";
    ((d["味わいタグ"]||"") + "," + (d["検索タグ"]||"")).split(',').forEach(t => { if(t.trim()) tagsHtml += `<span class="lq-tag">${t.trim()}</span>`; });
    let abv = d["度数"] || "-";
    if (parseFloat(abv) > 0 && parseFloat(abv) <= 1.0 && !abv.toString().includes('%')) abv = (parseFloat(abv) * 100).toFixed(0) + '%';

    let h = `<div class="label" style="justify-content:flex-end;">No.${d["No"]}</div><div class="lq-card">`;
    h += `<div class="lq-name">${(d["銘柄名"]||"").replace(/"/g, '')}</div>`;
    if(d["ソフィーのセリフ"]) h += `<div class="lq-quote">${d["ソフィーのセリフ"].replace(/^[「『"']|[」』"']$/g, '')}</div>`;
    h += `<div class="lq-basic-info"><div><span style="color:#e74c3c; font-size:0.8em">▶</span> ${d["大分類"]}　<span style="color:#3498db; font-size:0.8em">▶</span> ${d["中分類"]}</div><div><span style="color:#888">産地:</span> ${d["国"]} / ${d["産地"]}</div>${(d["製造元と創業年"] && d["製造元と創業年"] !== "-") ? `<div><span style="color:#888">蒸留所/製造:</span> ${d["製造元と創業年"]}</div>` : ""}${(d["公式URL"] && d["公式URL"] !== "-") ? `<div style="margin-top:6px;"><a href="${d["公式URL"]}" target="_blank" class="lq-btn-small">🔗 メーカーサイト</a></div>` : ""}</div>`;
    ...
    const cA4 = (d["中分類"] && AXIS4_MAP[d["中分類"]]) ? AXIS4_MAP[d["中分類"]] : AXIS4_DEFAULT;
    h += `<div style='font-size:0.65rem; color:var(--accent); margin-top:6px; margin-bottom:2px; text-align:center;'>${cA4.label}</div>${makeInlineGraph(cA4.left, cA4.right, "", "", d["Claude_第4軸"], true)}<div style="font-size:0.6rem; color:#888; text-align:right; margin-top:8px;"><span style="color:#10a37f">●GPT</span> <span style="color:#1a73e8">●Gem</span> <span style="color:#d97757">●Claude</span></div></div>`;
    h += `<div class="lq-specs-half"><div class="spec-row-compact"><span>知名度</span><span>${d["知名度"]}</span></div><div class="spec-row-compact"><span>度数</span><span>${abv}</span></div><div class="spec-row-compact"><span>誕生年</span><span>${d["銘柄誕生年"]}</span></div><div class="spec-row-compact"><span>市販</span><span>${d["市販価格"]}</span></div><div class="spec-row-compact"><span>Bar</span><span class="price-bar">${d["バー価格"]}</span></div></div></div>`; 
    ...
    const btnB = document.getElementById('ctrl-back');
    if(btnB) {
        if (nav.curG === null) { // nav.curGがnull = 検索モード
            btnB.innerText = '候補へ戻る'; btnB.style.fontSize = '0.8rem'; btnB.style.background = '#d35400'; btnB.style.color = '#fff'; btnB.style.border = '1px solid #e67e22';
        } else {
            btnB.innerText = 'リストへ'; btnB.style.fontSize = '0.8rem';
        }
    }
}
...
function resetCtrlBack() {
    const btnB = document.getElementById('ctrl-back');
    if(btnB) { btnB.innerText = '▲'; btnB.style.fontSize = ''; btnB.style.background = ''; btnB.style.color = ''; btnB.style.border = ''; }
}
...
function render(h, cb, isFS = false, conMode = 'standard') { 
    ...
    const lSide = document.querySelector('.l-side'); if(lSide) lSide.style.display = isFS ? 'none' : '';
    renderConsole(conMode);
    if(!isFS) resetCtrlBack();
}

function handleBack() {
    if (nav.state === "lq_card") {
        if(nav.curG === null && Array.isArray(nav.curP)) {
            // ★検索モード（curGがnull）なら、今のリスト(nav.curP)と位置(nav.curI)を維持して戻る
            renderSearchResults(nav.curP, nav.curI); 
        } else {
            // 普通のメニューモードなら、その中分類のリストに戻る
            openLiquorList(nav.curP); 
        }
    }
